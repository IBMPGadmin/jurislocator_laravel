...
<?php $__env->startSection('meta'); ?>
    ...
    <meta name="current-document-table" content="<?php echo e($table_name ?? ($tableInfo->table_name ?? '')); ?>">
    <meta name="current-document-category-id" content="<?php echo e($category_id ?? ($tableInfo->category_id ?? '')); ?>">
    <meta name="current-client-id" content="<?php echo e(isset($client) && $client ? $client->id : ''); ?>">
...
<?php $__env->startSection('main-content'); ?>
            ...
            <?php endif; ?>
            <?php if(empty($tableData) || (method_exists($tableData, 'count') ? $tableData->count() === 0 : count($tableData) === 0)): ?>
            ...
            <?php else: ?>
            <!-- Search Bar and Legal Content Section -->
            <div class="card mb-3 shadow-sm">
                <div class="card-header bg-light">
                    <h5 class="mb-0" data-en="Keyword Search" data-fr="Recherche par mots-clés">Keyword Search</h5>
                </div>
                <div class="card-body">
                    <form action="<?php echo e(route('french.legalTables.view', $table_name)); ?>" method="GET" class="mb-3">
                        <?php if(isset($client) && $client): ?>
            ...
            <div class="card mb-3 shadow-sm">
                <div class="widget_header card-header text-white" style="background-color: var(--color-theme-3);">
                ...
                <div class="card-body" id="legal-content-area">
                    ...
                    <?php
                        ...
                        // Debug: Add some output to see what we have
                        echo "<!-- DEBUG: tableData count: " . (is_countable($tableData) ? count($tableData) : 'not countable') . " -->";
                        if (method_exists($tableData, 'count')) {
                            echo "<!-- DEBUG: tableData->count(): " . $tableData->count() . " -->";
                        }
                        if (method_exists($tableData, 'items')) {
                            echo "<!-- DEBUG: tableData->items() count: " . count($tableData->items()) . " -->";
                        }
                    ...
                    <div class="legal-document-content">
                        ...
                        <?php if(empty($standaloneData) && empty($data) && !empty($tableData)): ?>
                            <div class="alert alert-info mb-3">
                                <strong>Debug: Raw Data Display</strong><br>
                                Found <?php echo e(method_exists($tableData, 'count') ? $tableData->count() : count($tableData)); ?> records in tableData.
                            </div>
                                ...
                                <div class="card mb-2">
                                    <div class="card-body">
                                        <h6 class="card-title">
                                            ...
                                            <?php else: ?>
                                                Entry <?php echo e($index + 1); ?>

                                            <?php endif; ?>
                                        </h6>
                                        <?php if(!empty($row->text_content)): ?>
                                            <div class="card-text">
                                                <?php echo makeLinksClickableSimple($row->text_content, $legalTable->id ?? $tableInfo->id, $row->section ?? null); ?>

                                            </div>
                                        <?php endif; ?>
                                        <?php if(!empty($row->footnote)): ?>
                                            <div class="footnote mt-2"><?php echo e($row->footnote); ?></div>
                                        <?php endif; ?>
                                        <small class="text-muted">
                                            Part: <?php echo e($row->part ?? 'N/A'); ?>, 
                                            Section: <?php echo e($row->section ?? 'N/A'); ?>, 
                                            Sub-section: <?php echo e($row->sub_section ?? 'N/A'); ?>
...
<?php echo $__env->make('layouts.with-sidebar-layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Ardent\Desktop\jurislocator new\jurislocator_laravel\resources\views/view-legal-table-french.blade.php ENDPATH**/ ?>