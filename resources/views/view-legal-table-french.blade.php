...
@section('meta')
    ...
    <meta name="current-document-table" content="{{ $table_name ?? ($tableInfo->table_name ?? '') }}">
    <meta name="current-document-category-id" content="{{ $category_id ?? ($tableInfo->category_id ?? '') }}">
    <meta name="current-client-id" content="{{ isset($client) && $client ? $client->id : '' }}">
...
@section('main-content')
            ...
            @endif
            @if(empty($tableData) || (method_exists($tableData, 'count') ? $tableData->count() === 0 : count($tableData) === 0))
            ...
            <div class="card mb-3 shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('french.legalTables.view', $table_name) }}" method="GET" class="mb-3">
                        @if(isset($client) && $client)
            ...
            <div class="card mb-3 shadow-sm">
                ...
                <div class="card-body" id="legal-content-area">
                    ...
                    <div class="legal-document-content">
                        {{-- Debug output --}}
                        @if(empty($standaloneData) && empty($data) && !empty($tableData))
                            <div class="alert alert-info mb-3">
                                <strong>Debug: Raw Data Display</strong><br>
                                Found {{ method_exists($tableData, 'count') ? $tableData->count() : count($tableData) }} records in tableData.
                            </div>
                                ...
                                <div class="card mb-2">
                                    <div class="card-body">
                                        ...
                                        @if(!empty($row->text_content))
                                            <div class="card-text">
                                                {!! makeLinksClickableSimple($row->text_content, $legalTable->id ?? $tableInfo->id, $row->section ?? null) !!}
                                            </div>
                                        @endif
                                        @if(!empty($row->footnote))
                                            <div class="footnote mt-2">{{ $row->footnote }}</div>
                                        @endif
                                        <small class="text-muted">
                                            Part: {{ $row->part ?? 'N/A' }}, 
                                            Section: {{ $row->section ?? 'N/A' }}, 
                                            Sub-section: {{ $row->sub_section ?? 'N/A' }}
                                        </small>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        @endforeach
                        @endif {{-- End fallback data display --}}
                    </div>
                </div>
            </div>
