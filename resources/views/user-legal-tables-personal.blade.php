...
<div class="container">
    <div class="row gap_top">
                            ...
                            <div class="d-flex flex-wrap">
                                <div class="me-4 mb-2">
                                    <span class="d-flex align-items-center">
                                        <strong data-en="User:" data-fr="Utilisateur :">User:</strong>&nbsp;{{ auth()->user()->name ?? '-' }}
                                    </span>
                                </div>
                                <div class="mb-2">
                                    <span class="d-flex align-items-center">
                                        <i class="fas fa-envelope me-2 text-secondary"></i>
                                        <strong data-en="Email:" data-fr="Courriel :">Email:</strong>&nbsp;{{ auth()->user()->email ?? '-' }}
                                    </span>
                                </div>
                            </div>
                        </div>
        ...
        <form method="GET" action="{{ route('user.legal-tables') }}" id="filterForm" class="gap_top btn-shadow bg_custom p-4 rounded shadow-sm">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-lg-12 d-flex submit_reset_format justify-content-end">
                    <div class="button-group">
                        ...
                        <a href="{{ route('user.legal-tables') }}" class="btn btn-reset">
    ...
    <div class="row">
        ...
        <div class="row gap_top custom-container act-content">
            <div class="act-content grid-view">
                @if($results->count())
                    @foreach($results as $row)                        
                        <div class="col-lg-4 col-md-6 act-card btn-shadow" onclick="redirectToUserDocument('{{ $row->table_name }}', '{{ $row->id }}', '{{ $row->language_id ?? $row->language ?? '' }}')">
            ...
            <div class="act-content list-view" style="display: none;">
                @if($results->count())
                    @foreach($results as $row)                        
                        <div class="col-lg-12 act-card btn-shadow" onclick="redirectToUserDocument('{{ $row->table_name }}', '{{ $row->id }}', '{{ $row->language_id ?? $row->language ?? '' }}')">
                            <div class="act-card-inner">
                                <div class="act-home-title">{{ $row->act_name }}</div>
                                <div class="act-language" style="color: red;"><span data-en="Language:" data-fr="Langue :">Language:</span> 
                                    @php
