...
<div class="container">
            ...
            <div class="row g-3 align-items-center">
                ...
                <div class="col-lg-12 d-flex submit_reset_format justify-content-end">
                    <div class="button-group">
                        ...
                        <button type="button" class="btn btn-reset" onclick="window.location.href='{{ url()->current() }}'">
    ...
    <h2 class="gap_top">Client: {{ $client->client_name ?? 'N/A' }}</h2>
    <h3>Legal Tables</h3>
    <div class="row">
        ...
        @else
            @foreach($legalTables as $table)
                <div class="col-lg-4 col-md-6 act-card btn-shadow" onclick="redirectToDocument('{{ $table->table_name }}', '{{ $table->id }}', '{{ $client->id ?? request('client_id') }}', '{{ $table->language_id ?? $table->language ?? '' }}')">
                    <div class="act-card-inner">
                        <i class="fas fa-book act-icon"></i>
                        <div class="act-home-title">{{ $table->act_name }}</div>
                        <div class="act-category">Category: {{ $acts[$table->act_id] ?? $table->act_id }}</div>
                        <div class="act-law">Law Subject: {{ $lawSubjects[$table->law_id] ?? $table->law_id }}</div>
                        <div class="act-jurisdiction">Jurisdiction: {{ $jurisdictions[$table->jurisdiction_id] ?? $table->jurisdiction_id }}</div>
                        ...
                        @endif
                        <div class="act-description">Created: {{ $table->created_at }}</div>
