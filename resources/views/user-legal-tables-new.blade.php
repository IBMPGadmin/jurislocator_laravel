                    ...
                    <div id="toggleTileContainer" class="row toggle-tile-warpper sp-top">
                        @if($results->count())
                            ...
                            <div class="toggle-tile-body col-12 col-md-4 col-lg-3 grid">
                                <div class="toggle-tile-content shadow-sm sp-top" data-en="{{ $row->act_name }}" data-fr="{{ $row->act_name }}" onclick="redirectToDocument('{{ $row->table_name }}', '{{ $row->id }}', '{{ $client->id }}', '{{ $row->language_id ?? $row->language ?? '' }}')">
                                    <h4><i class="fas fa-book act-icon"></i> {{ $row->act_name }}</h4>
                                    <ul class="act-data">
                                        <li class="act-category"><strong data-en="Category: " data-fr="Catégorie: ">Category: </strong><span>{{ $acts[$row->act_id] ?? $row->act_id }}</span></li>
                                        <li class="act-law"><strong data-en="Law Subject: " data-fr="Sujet de droit: ">Law Subject: </strong><span>{{ $lawSubjects[$row->law_id] ?? $row->law_id }}</span></li>
                                        <li class="act-jurisdiction"><strong data-en="Jurisdiction: " data-fr="Juridiction: ">Jurisdiction: </strong><span>{{ $jurisdictions[$row->jurisdiction_id] ?? $row->jurisdiction_id }}</span></li>
