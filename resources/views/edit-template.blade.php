...
@section('meta')
    ...
    <meta name="current-client-id" content="{{ $client->id ?? 0 }}">
        ...
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fas fa-edit me-2"></i> Edit Template: {{ $templateTitle ?? 'Document Template' }}</h5>
                    <div>
                        <a href="{{ route('templates.index', ['client_id' => $client->id ?? 1]) }}" class="btn btn-light btn-sm me-2">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4 p-3 bg-light rounded">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Client:</strong> {{ $client->client_name ?? 'No client selected' }}</p>
                                <p class="mb-1"><strong>Email:</strong> {{ $client->client_email ?? 'N/A' }}</p>
                            </div>
                            <div class="col-md-6 text-md-end">
                                <p class="mb-1"><strong>Date:</strong> {{ now()->format('F d, Y') }}</p>
                                <p class="mb-1"><strong>Reference:</strong> #{{ $client->id ?? '000' }}-{{ now()->format('Ymd') }}</p>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Template Editor -->
                    <form id="template-form" action="{{ route('templates.save', ['client_id' => $client->id ?? 1, 'template_id' => $templateId ?? 1]) }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <input type="text" class="form-control" id="subject" name="subject" placeholder="Email Subject" 
                                   value="{{ $subject ?? 'Re: Your Immigration Application - ' . ($client->client_name ?? 'Client') }}">
                        </div>
                        
                        <textarea id="template-editor" name="content" class="form-control">
                            {!! $templateContent ?? getDefaultTemplateContent($templateId ?? 1, $client->client_name ?? 'Client') !!}
                        </textarea>
                    </form>
                </div>
            </div>
    ...
    tinymce.init({
        ...
        setup: function(editor) {
            editor.on('init', function() {
                ...
                content = content.replace(/{client_name}/g, "{{ $client->client_name ?? 'Client' }}");
                content = content.replace(/{client_email}/g, "{{ $client->client_email ?? 'client@example.com' }}");
                content = content.replace(/{reference}/g, "#{{ $client->id ?? '000' }}-{{ now()->format('Ymd') }}");
...
function replacePlaceholders(content, client) {
    return content
        ...
        .replace(/{user_name}/g, '{{ Auth::user()->name ?? "User Name" }}')
        .replace(/{user_email}/g, '{{ Auth::user()->email ?? "user@example.com" }}');
}
</script>
@endpush
