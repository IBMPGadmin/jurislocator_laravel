            ...
            <div class="bg_custom p-4 rounded shadow-sm">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="session-actions">
                        ...
                        <a href="{{ route('session.mode.selection') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="main-content col-lg-8 col-md-7">
            ...
            <div class="row gap_top custom-container act-content">
                <div class="act-content grid-view">
                    @if($results->count())
                        @foreach($results as $row)
                            <div class="col-lg-4 col-md-6 act-card btn-shadow" onclick="redirectToUserDocument('{{ $row->table_name }}', '{{ $row->id }}', '{{ $row->language_id ?? $row->language ?? null }}')">
                    ...
                    @else
                        <div class="col-12 no-results">
                            ...
                            <a href="{{ route('user.legal-tables') }}" class="btn btn-primary mt-2">
                ...
                <div class="act-content list-view" style="display: none;">
                    @if($results->count())
                        @foreach($results as $row)
                            <div class="col-lg-12 act-card btn-shadow" onclick="redirectToUserDocument('{{ $row->table_name }}', '{{ $row->id }}', '{{ $row->language_id ?? $row->language ?? null }}')">
                                <div class="act-card-inner">
                                    <div class="act-home-title">{{ $row->act_name }}</div>
                                    <div class="act-language" style="color: red;"><span data-en="Language:" data-fr="Langue :">Language:</span> 
                                        @php
